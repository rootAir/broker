...
app.conf.update(
                BROKER_URL = broker.get('BROKER_URL_AMQP') or "amqp://guest:guest@localhost:5672//",
                BROKER_BACKEND = "amqp",
...
class syncBroker(syncBroker):
    def __init__(self, *args, **kwargs):
        super(self.__class__, self).__init__(*args, **kwargs)


class findOptionsLoop(findOptionsLoop):
    def __init__(self, *args, **kwargs):
        super(self.__class__, self).__init__(*args, **kwargs)
...
# @shared_task
@app.task  #(queue='find_options_loop')
def find_options_loop():
    find_options = findOptionsLoop()
    find_options.run()